...
def graph2seq(pid, args, st, ed, nids, graph_data, sequence_array):
    seq_loader = data_utils.GroupFeatureSequenceLoader(
        graph_data, fanouts=args['fanouts'], grp_norm=args['grp_norm']
    )
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--train_size', type=float, default=0.4, help='Train size of nodes.'
    )
    parser.add_argument(
        '--val_size', type=float, default=0.1, help='Val size of nodes.'
    )
    parser.add_argument(
        '--seed', type=int, default=717, help='Collecting neighbots in n hops.'
    )
    ...
    parser.add_argument('--n_workers', type=int, default=1, help='Using n processes.')
    ...
    file_name = (
        f"{args['dataset']}_{flag_1}_{flag_2}_{n_hops}_"
    ...
    sequence_array = np.memmap(
        seq_file, dtype=np.float32, mode='w+', shape=(n_nodes, seq_len, feat_dim)
    )
    ...
    for pid in range(n_workers):
        ...
        p = mp.Process(
            target=graph2seq,
            args=(pid, args, st, ed, nids[st:ed], graph_data, sequence_array),
        )
        procs.append(p)
        p.start()
